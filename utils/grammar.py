...
class NGram(Grammar):
    ...
    def __init__(self, transcript_file, label2index_map, ngram_order):
        ...
        self.ngrams, self.vocabulary = self._get_statistics(transcript_file, label2index_map)
    ...
    def _get_statistics(self, transcript_file, label2index_map):
        ...
        with open(transcript_file, 'r') as f:
            lines = f.read().split('\n')[0:-1]
        for line in lines:
            labels = [ self.start_symbol() ] + [ label2index_map[label] for label in line.split() ] + [ self.end_symbol() ]
                ...
                if len(key) == order + 1:
                    ...
                    for w in self.vocabulary:
                        if not context + (w,) in self.ngrams:
                            h = tuple(context[0:-1])
                            self.normalization[key] = self.normalization.get(key, 0) + self._probability(h, w)
    ...
    def _probability(self, context, label):
        if context + (label,) in self.ngrams:
            ...
            p = self._probability(tuple(context[:-1]), context[-1]) / self.normalization.get(context + (label,), 1)
